...
add-highlighter shared/lisp/comment-form region -recurse "\(" "#;\(" "\)" fill comment
...
add-highlighter shared/lisp/code/ regex [\s\(\)](defun|defmacro|defpackage|defgeneric|defmethod|defstruct|defclass|in-package|defvar|defparameter)[\s\(\)] 1:keyword
add-highlighter shared/lisp/code/ regex [\s\(\)](if|cond|when|case)[\s\(\)] 1:keyword
add-highlighter shared/lisp/code/ regex [\s\(\)](lambda|let|let\*|progn|map|loop)[\s\(\)] 1:keyword
add-highlighter shared/lisp/code/ regex [\s\(\)](null|consp)[\s\(\)] 1:keyword
add-highlighter shared/lisp/code/ regex [\s\(\)](error)[\s\(\)] 1:keyword
# add-highlighter shared/lisp/code/ regex [\s\(\)](or|and|not)[\s\(\)] 1:keyword
